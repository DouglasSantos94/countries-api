...
const getCountries = (): Promise<AxiosResponse> =>
  axios.get(
    "https://restcountries.com/v2/all?fields=name,population,region,subregion,capital,topLevelDomain,currencies,languages,flag,borders,alpha3code"
  );
...
const getCountriesByRegion = (regionFilter: string): Promise<AxiosResponse> =>
  axios.get(`https://restcountries.com/v2/region/${regionFilter}`);

const getCountryBorders = (borders: string[]): Promise<string[]> =>
  Promise.all(
    borders.map((b) =>
      axios
        .get(`https://restcountries.com/v2/alpha/${b}`)
        .then(({ data: { name } }) => name)
